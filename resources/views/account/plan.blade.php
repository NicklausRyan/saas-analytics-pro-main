            ...
            <div class="row m-n2">
                <div class="col-12 col-md-6 p-2 d-flex align-items-center">
                    ...
                    <div class="{{ ($user->plan->features->pageviews == 0 ? 'text-muted' : '') }} {{ (__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-3') }}">
                        ...
                        @else($user->plan->features->pageviews)
                            <span class="text-muted">{{ $pageviewsCount }} /</span> {{ __(($user->plan->features->pageviews == 1 ? ':number pageview' : ':number pageviews'), ['number' => number_format($user->plan->features->pageviews, 0, __('.'), __(','))]) }}
                        @endif
                    </div>
                ...
                <div class="col-12 col-md-6 p-2 d-flex align-items-center">
                    ...
                    <div class="{{ ($user->plan->features->websites == 0 ? 'text-muted' : '') }} {{ (__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-3') }}">
                        ...
                        @else($user->plan->features->websites)
                            {{ __(($user->plan->features->websites == 1 ? ':number website' : ':number websites'), ['number' => number_format($user->plan->features->websites, 0, __('.'), __(','))]) }}
                        @endif
                    </div>
                </div>
