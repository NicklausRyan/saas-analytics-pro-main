            ...
            <div class="card-body py-4">
                ...
                @else
                    <div class="list-group list-group-flush my-n3">
                        ...
                        @php
                            $osRevenue = 0;
                            $osRevenuePerVisitor = 0;
                            
                            // Find this OS in the revenue data
                            foreach($revenueByOperatingSystem as $revenueData) {
                                if($revenueData['value'] == $operatingSystem->value) {
                                    $osRevenue = $revenueData['revenue'];
                                    $osRevenuePerVisitor = $revenueData['revenuePerVisitor'];
                                    break;
                                }
                            }
                        @endphp
                        <div class="list-group-item px-0 border-0 position-relative" style="z-index: 2; margin: 0;">
                            <div class="d-flex align-items-center position-relative" style="z-index: 3; min-height: 44px;">
                                ...
                                <!-- Revenue Column -->
                                <div style="width: 27.5%; flex-shrink: 0; text-align: right; padding: 0 8px;" class="text-{{ $osRevenue > 0 ? 'success font-weight-medium' : 'muted' }}">
                                    {{ $osRevenue ? number_format($osRevenue, 2, __('.'), __(',')) . ' ' . $primaryCurrency : '—' }}
                                </div>
                            </div>
                            ...
                            <div class="position-absolute" style="top: 50%; left: 0; width: 50%; height: 32px; transform: translateY(-50%); z-index: 1; pointer-events: none;">
                                @php
                                    ...
                                    $visitorsPercentage = ($operatingSystem->count / $total->count) * 100;
                                    $maxOSRevenue = is_array($revenueByOperatingSystem) ? 
                                        max(array_column($revenueByOperatingSystem, 'revenue')) : 
                                        collect($revenueByOperatingSystem)->max('revenue');
                                    $osRevenuePercentage = $maxOSRevenue > 0 ? (($osRevenue / $maxOSRevenue) * 100) : 0;
                                      
                                    // Determine which bar is smaller and should be on top
                                    $visitorsIsSmaller = $visitorsPercentage <= $osRevenuePercentage;
                                    $smallerPercentage = $visitorsIsSmaller ? $visitorsPercentage : $osRevenuePercentage;
                                    $largerPercentage = $visitorsIsSmaller ? $osRevenuePercentage : $visitorsPercentage;
