            ...
            <div class="card-header">
                <div class="row no-gutters">
                    <div class="col-12 col-md">
                        <div class="font-weight-medium py-1">{{ __('Landing pages') }}</div>
                    </div>
                    ...
                    <div class="list-group list-group-flush my-n3">
                        ...
                        <div class="list-group-item px-0 small text-muted">
                            <div class="d-flex align-items-center">
                                ...
                                <div style="width: 27.5%; flex-shrink: 0; text-align: right; padding: 0 8px;">
                                    {{ number_format($totalLandingPagesRevenue ?? 0, 2, __('.'), __(',')) }} {{ $primaryCurrency ?? '' }}
                                </div>
                            </div>
                        </div>
                        @foreach($landingPages as $landingPage)
                            @php
                                $landingPageRevenue = 0;
                                $landingPageRevenuePerVisitor = 0;
                                
                                // Always find this landing page in the revenue data
                                foreach($revenueByLandingPage as $revenueData) {
                                    if($revenueData['value'] == $landingPage->value) {
                                        $landingPageRevenue = $revenueData['revenue'];
                                        $landingPageRevenuePerVisitor = $revenueData['revenuePerVisitor'];
                                        break;
                                    }
                                }
                            @endphp
                            <div class="list-group-item px-0 border-0 position-relative" style="z-index: 2; margin: 0;">
                                <div class="d-flex align-items-center position-relative" style="z-index: 3; min-height: 44px;">
                                    ...
                                    <div style="width: 45%; flex-shrink: 0; padding: 0 12px;">
                                        <div class="d-flex text-truncate align-items-center">
                                            <div class="text-truncate" dir="ltr">{{ $landingPage->value }}</div>
                                            <a href="http://{{ $website->domain . $landingPage->value }}" target="_blank" rel="nofollow noreferrer noopener" class="text-secondary d-flex align-items-center {{ (__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-2') }}">@include('icons.open-in-new', ['class' => 'fill-current width-3 height-3'])</a>
                                        </div>
                                    </div>
                                    <!-- Visitors Column -->
                                    <div style="width: 27.5%; flex-shrink: 0; text-align: right; padding: 0 8px;">
                                    ...
                                    <!-- Revenue Column -->
                                    <div style="width: 27.5%; flex-shrink: 0; text-align: right; padding: 0 8px;" class="text-{{ $landingPageRevenue > 0 ? 'success font-weight-medium' : 'muted' }}">
                                        {{ $landingPageRevenue ? number_format($landingPageRevenue, 2, __('.'), __(',')) . ' ' . ($primaryCurrency ?? '') : '-' }}
                                    </div>
                                </div>
                                ...
                                <div class="position-absolute" style="top: 50%; left: 0; width: 50%; height: 32px; transform: translateY(-50%); z-index: 1; pointer-events: none;">
                                    @php
                                        ...
                                        $visitorsPercentage = ($landingPage->count / $total->count) * 100;
                                        $maxLandingPageRevenue = is_array($revenueByLandingPage) ? 
                                            max(array_column($revenueByLandingPage, 'revenue')) : 
                                            collect($revenueByLandingPage)->max('revenue');
                                        $landingPageRevenuePercentage = $maxLandingPageRevenue > 0 ? (($landingPageRevenue / $maxLandingPageRevenue) * 100) : 0;
                                          
                                        // Determine which bar is smaller and should be on top
                                        $visitorsIsSmaller = $visitorsPercentage <= $landingPageRevenuePercentage;
                                        $smallerPercentage = $visitorsIsSmaller ? $visitorsPercentage : $landingPageRevenuePercentage;
                                        $largerPercentage = $visitorsIsSmaller ? $landingPageRevenuePercentage : $visitorsPercentage;
                                          
                                        // Add spacing between smaller and larger bars
                                        $spacingGap = 0.5859375;
                                        $largerBarStartPosition = $smallerPercentage + $spacingGap;
                                    @endphp
                                    @if($landingPageRevenue > 0)
