                    ...
                    <div class="list-group list-group-flush my-n3">
                        ...
                        @foreach($socialNetworks as $socialNetwork)
                            @php
                                ...
                                $socialNetworkRevenuePerVisitor = 0;
                                
                                // Always find this social network in the revenue data
                                foreach($revenueBySocialNetwork as $revenueData) {
                                    if($revenueData['value'] == $socialNetwork->value) {
                                        $socialNetworkRevenue = $revenueData['revenue'];
                                        $socialNetworkRevenuePerVisitor = $revenueData['revenuePerVisitor'];
                                        break;
                                    }
                                }
                            @endphp
                            ...
                            <div class="list-group-item px-0 border-0 position-relative" style="z-index: 2; margin: 0;">
                                <div class="d-flex align-items-center position-relative" style="z-index: 3; min-height: 44px;">
                                    ...
                                    <div style="width: 45%; flex-shrink: 0; padding: 0 12px;">
                                        <div class="d-flex text-truncate align-items-center">
                                            <div class="d-flex align-items-center {{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}">
                                                <img src="https://icons.duckduckgo.com/ip3/{{ $socialNetwork->value }}.ico" rel="noreferrer" class="width-4 height-4">
                                            </div>
                                            <div class="text-truncate" dir="ltr">{{ $socialNetwork->value }}</div>
                                        </div>
                                    </div>
                                    <!-- Visitors Column -->
                                    <div style="width: 27.5%; flex-shrink: 0; text-align: right; padding: 0 8px;">
                                    ...
                                    <!-- Revenue Column -->
                                    <div style="width: 27.5%; flex-shrink: 0; text-align: right; padding: 0 8px;" class="text-{{ $socialNetworkRevenue > 0 ? 'success font-weight-medium' : 'muted' }}">
                                        {{ $socialNetworkRevenue ? number_format($socialNetworkRevenue, 2, __('.'), __(',')) . ' ' . $primaryCurrency : '-' }}
                                    </div>
                                </div>
                                ...
                                <div class="position-absolute" style="top: 50%; left: 0; width: 50%; height: 32px; transform: translateY(-50%); z-index: 1; pointer-events: none;">
                                    @php
                                        ...
                                        $visitorsPercentage = ($socialNetwork->count / $total->count) * 100;
                                        $maxSocialNetworkRevenue = is_array($revenueBySocialNetwork) ? 
                                            max(array_column($revenueBySocialNetwork, 'revenue')) : 
                                            collect($revenueBySocialNetwork)->max('revenue');
                                        $socialNetworkRevenuePercentage = $maxSocialNetworkRevenue > 0 ? (($socialNetworkRevenue / $maxSocialNetworkRevenue) * 100) : 0;
                                          
                                        // Determine which bar is smaller and should be on top
                                        $visitorsIsSmaller = $visitorsPercentage <= $socialNetworkRevenuePercentage;
                                        $smallerPercentage = $visitorsIsSmaller ? $visitorsPercentage : $socialNetworkRevenuePercentage;
                                        $largerPercentage = $visitorsIsSmaller ? $socialNetworkRevenuePercentage : $visitorsPercentage;
