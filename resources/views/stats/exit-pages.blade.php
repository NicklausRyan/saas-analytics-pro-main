@section('site_title', formatTitle([$website->domain, __('Exit pages'), config('settings.title')]))
...
</div><div class="row m-n2">
    <div class="col-12 p-3">        <div class="card border-0 shadow-sm"><div class="card-header">
                <div class="row no-gutters">
                    <div class="col-12 col-md"><div class="font-weight-medium py-1">{{ __('Exit pages') }}</div></div>
                ...
                @else                    <div class="list-group list-group-flush my-n3"><div class="list-group-item px-0 text-muted">
                            ...
                            <div class="d-flex align-items-center">
                                ...
                                <div style="width: 27.5%; flex-shrink: 0; text-align: right; padding: 0 8px;">
                                    {{ number_format($totalExitPagesRevenue ?? 0, 2, __('.'), __(',')) }} {{ $primaryCurrency ?? '' }}
                                </div>
                            </div>
                        </div>@foreach($exitPages as $exitPage)
                            @php
                                $exitPageRevenue = 0;
                                $exitPageRevenuePerVisitor = 0;
                                
                                // Always find this exit page in the revenue data
                                foreach($revenueByExitPage as $revenueData) {
                                    if($revenueData['value'] == $exitPage->value) {
                                        $exitPageRevenue = $revenueData['revenue'];
                                        $exitPageRevenuePerVisitor = $revenueData['revenuePerVisitor'];
                                        break;
                                    }
                                }
                            @endphp                            <div class="list-group-item px-0 border-0 position-relative" style="z-index: 2; margin: 0;">                                <div class="d-flex align-items-center position-relative" style="z-index: 3; min-height: 44px;">
                                    ...
                                    <div style="width: 45%; flex-shrink: 0; padding: 0 12px;">
                                        <div class="d-flex text-truncate align-items-center">
                                            <div class="text-truncate" dir="ltr">{{ $exitPage->value }}</div>
                                            <a href="http://{{ $website->domain . $exitPage->value }}" target="_blank" rel="nofollow noreferrer noopener" class="text-secondary d-flex align-items-center {{ (__('lang_dir') == 'rtl' ? 'mr-2' : 'ml-2') }}">@include('icons.open-in-new', ['class' => 'fill-current width-3 height-3'])</a>
                                        </div>
                                    </div>
                                    <!-- Visitors Column -->
                                    <div style="width: 27.5%; flex-shrink: 0; text-align: right; padding: 0 8px;">
                                        {{ number_format($exitPage->count, 0, __('.'), __(',')) }}
                                    </div>
                                    <!-- Revenue Column -->
                                    <div style="width: 27.5%; flex-shrink: 0; text-align: right; padding: 0 8px;" class="text-{{ $exitPageRevenue > 0 ? 'success font-weight-medium' : 'muted' }}">
                                        {{ $exitPageRevenue ? number_format($exitPageRevenue, 2, __('.'), __(',')) . ' ' . ($primaryCurrency ?? '') : '-' }}
                                    </div>
                                ...
                                <div class="position-absolute" style="top: 50%; left: 0; width: 50%; height: 32px; transform: translateY(-50%); z-index: 1; pointer-events: none;">
                                    @php
                                        ...
                                        $visitorsPercentage = ($exitPage->count / $total->count) * 100;
                                        $maxExitPageRevenue = is_array($revenueByExitPage) ? 
                                            max(array_column($revenueByExitPage, 'revenue')) : 
                                            collect($revenueByExitPage)->max('revenue');
                                        $exitPageRevenuePercentage = $maxExitPageRevenue > 0 ? (($exitPageRevenue / $maxExitPageRevenue) * 100) : 0;
                                          
                                        // Determine which bar is smaller and should be on top
                                        $visitorsIsSmaller = $visitorsPercentage <= $exitPageRevenuePercentage;
                                        $smallerPercentage = $visitorsIsSmaller ? $visitorsPercentage : $exitPageRevenuePercentage;
                                        $largerPercentage = $visitorsIsSmaller ? $exitPageRevenuePercentage : $visitorsPercentage;
                        ...
                        <div class="mt-3 align-items-center">
                            <div class="row">
                                <div class="col">
                                    <div class="mt-2 mb-3">{{ __('Showing :from-:to of :total', ['from' => $exitPages->firstItem(), 'to' => $exitPages->lastItem(), 'total' => $exitPages->total()]) }}
                                    </div>
                                </div>
                                <div class="col-auto">
                                    {{ $exitPages->onEachSide(1)->links() }}
                                </div>
                            </div>
                        </div>
                    </div>
