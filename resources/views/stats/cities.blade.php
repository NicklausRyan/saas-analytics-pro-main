@section('site_title', formatTitle([$website->domain, __('Cities'), config('settings.title')]))
                    ...
                    @if(isset($first->value))
                        ...
                        <div class="flex-grow-1 font-weight-bold text-truncate {{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}">
                            @if(formatCountryName($first->value) !== __('Unknown'))
                                <span data-tooltip="true" title="{{ __(formatCountryName($first->value)) }}">{{ formatCountryName($first->value) }}</span>
                            @else
                                {{ __('Unknown') }}
                            @endif
                        </div>
    ...
    <div class="col-12 p-3">
        <div class="card border-0 shadow-sm">
            ...
            @else<div class="list-group list-group-flush my-n3">
                    ...
                    <div class="list-group-item px-0 small text-muted">
                    ...
                    </div>                    @foreach($cities as $city)
                        @php
                            $cityRevenue = 0;
                            $cityRevenuePerVisitor = 0;
                              // Always find this city in the revenue data
                            foreach($revenueByCity as $revenueData) {
                                if($revenueData['value'] == $city->value) {
                                    $cityRevenue = $revenueData['revenue'];
                                    $cityRevenuePerVisitor = $revenueData['revenuePerVisitor'];
                                    break;
                                }
                            }
                        @endphp
                        ...
                        <div class="list-group-item px-0 border-0 position-relative" style="z-index: 2; margin: 0;">
                            <div class="d-flex align-items-center position-relative" style="z-index: 3; min-height: 44px;">
                                ...
                                <div style="width: 45%; flex-shrink: 0; padding: 0 12px;">
                                    <div class="d-flex text-truncate align-items-center">
                                        <div class="d-flex align-items-center {{ (__('lang_dir') == 'rtl' ? 'ml-2' : 'mr-2') }}">
                                            <img src="{{ asset('/images/icons/countries/'. formatFlag($city->value)) }}.svg" class="width-4 height-4">
                                        </div>
                                ...
                                <!-- Revenue Column -->
                                <div style="width: 27.5%; flex-shrink: 0; text-align: right; padding: 0 8px;" class="text-{{ $cityRevenue > 0 ? 'success font-weight-medium' : 'muted' }}">
                                    {{ $cityRevenue ? number_format($cityRevenue, 2, __('.'), __(',')) . ' ' . $primaryCurrency : '-' }}
                                </div>
                            </div>
                            ...
                            <div class="position-absolute" style="top: 50%; left: 0; width: 50%; height: 32px; transform: translateY(-50%); z-index: 1; pointer-events: none;">
                                @php
                                    ...
                                    $visitorsPercentage = ($city->count / $total->count) * 100;                                    $maxCityRevenue = is_array($revenueByCity) ? 
                                        max(array_column($revenueByCity, 'revenue')) : 
                                        (is_object($revenueByCity) ? collect($revenueByCity)->max('revenue') : 0);
                                    $cityRevenuePercentage = $maxCityRevenue > 0 ? (($cityRevenue / $maxCityRevenue) * 100) : 0;
                                      
                                    // Determine which bar is smaller and should be on top
                                    $visitorsIsSmaller = $visitorsPercentage <= $cityRevenuePercentage;
                                    $smallerPercentage = $visitorsIsSmaller ? $visitorsPercentage : $cityRevenuePercentage;
                                    $largerPercentage = $visitorsIsSmaller ? $cityRevenuePercentage : $visitorsPercentage;
