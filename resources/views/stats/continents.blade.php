    ...
    <div class="col-12 p-3">
        <div class="card border-0 shadow-sm">
            ...
            @else                <div class="list-group list-group-flush my-n3">
                    ...
                    <div class="list-group-item px-0 small text-muted">
                    ...
                    </div>                    @foreach($continents as $continent)
                        @php
                            $continentRevenue = 0;
                            $continentRevenuePerVisitor = 0;
                              // Always find this continent in the revenue data
                            foreach($revenueByContinent as $revenueData) {
                                if($revenueData['value'] == $continent->value) {
                                    $continentRevenue = $revenueData['revenue'];
                                    $continentRevenuePerVisitor = $revenueData['revenuePerVisitor'];
                                    break;
                                }
                            }
                        @endphp
                        ...
                        <div class="list-group-item px-0 border-0 position-relative" style="z-index: 2; margin: 0;">
                            <div class="d-flex align-items-center position-relative" style="z-index: 3; min-height: 44px;">
                                ...
                                <!-- Revenue Column -->
                                <div style="width: 27.5%; flex-shrink: 0; text-align: right; padding: 0 8px;" class="text-{{ $continentRevenue > 0 ? 'success font-weight-medium' : 'muted' }}">
                                    {{ $continentRevenue ? number_format($continentRevenue, 2, __('.'), __(',')) . ' ' . $primaryCurrency : '-' }}
                                </div>
                            </div>
                            ...
                            <div class="position-absolute" style="top: 50%; left: 0; width: 50%; height: 32px; transform: translateY(-50%); z-index: 1; pointer-events: none;">
                                @php
                                    ...
                                    $visitorsPercentage = ($continent->count / $total->count) * 100;                                    $maxContinentRevenue = is_array($revenueByContinent) ? 
                                        max(array_column($revenueByContinent, 'revenue')) : 
                                        (is_object($revenueByContinent) ? collect($revenueByContinent)->max('revenue') : 0);
                                    $continentRevenuePercentage = $maxContinentRevenue > 0 ? (($continentRevenue / $maxContinentRevenue) * 100) : 0;
                                      
                                    // Determine which bar is smaller and should be on top
                                    $visitorsIsSmaller = $visitorsPercentage <= $continentRevenuePercentage;
                                    $smallerPercentage = $visitorsIsSmaller ? $visitorsPercentage : $continentRevenuePercentage;
                                    $largerPercentage = $visitorsIsSmaller ? $continentRevenuePercentage : $visitorsPercentage;
